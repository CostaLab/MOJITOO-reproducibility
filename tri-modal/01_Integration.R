...
if(UPDATE_VEC["Signac"] == 1){
  ...
  multiome <- SignacProcess(object=multiome,
                           ASSAY_list=list(cf$ASSAY_1,cf$ASSAY_2,cf$ASSAY_3),
                           nredu_list=list(cf$Redu_1,cf$Redu_2,cf$Redu_3),
                           dims_list=list(cf$dims_1,cf$dims_2,cf$dims_3),
                           name = dataset
  )
}
...
if(UPDATE_VEC["WNN"] == 1){
  message(">>>>>>>>>>>>>>>>>>>>>>>WNN ", date())
  a_bench <- bench::mark(
  multiome <- FindMultiModalNeighbors(multiome,
                                      reduction.list = list(cf$Redu_1, cf$Redu_2, cf$Redu_3),
                                      dims.list = list(cf$dims_1, cf$dims_2, cf$dims_3)) %>%
              RunUMAP(nn.name = "weighted.nn", reduction.name = "WNN_UMAP", reduction.key = "wnnUMAP_", verbose=F)
  )
  a_bench$result <- NULL
